...
"""
Go to a fixed return position (roughly in the middle),
Turn towards the player while ball isn't active, then turn towards ball as it comes towards you .
How to move towards ball -> Wait until the bounce: 
"""
...
func move_to_ball(delta):
	...
	var dist = Vector2(actor.global_position.x, actor.global_position.z) - ps
	if sqrt(dist.x*dist.x + dist.y + dist.y) > 0.05:
		actor.curr_target = ps
		velocity_ = actor.velocity
	else:
		actor.curr_target = null
...
func predict_bounce(ball: TennisBall, delta, max_time = 2.0):
	...
	while t < max_time:
		...
		if bounced and 0.3 <= pos.y and pos.y <= 0.4:
